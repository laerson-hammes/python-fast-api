...
import pytest
from api_requests.api import app, recuperar_itens_por_pedido
from fastapi.testclient import TestClient
from uuid import UUID
from api_requests.schema import Item
from api_requests.exceptions import PedidoNaoEncontradoError, FalhaDeComunicacaoError
...
class TestListarPedidos:
   def test_quando_identificacao_do_pedido_invalido_um_erro_deve_ser_retornado(self, client):
      resposta = client.get("/orders/valor-invalido/items")
      assert resposta.status_code == HTTPStatus.UNPROCESSABLE_ENTITY


   def test_quando_pedido_nao_encontrado_um_erro_deve_ser_retornado(self, client, sobreescreve_recuperar_itens_por_pedido):
   ...
   def test_quando_encontrar_pedido_codigo_ok_deve_ser_retornado(self, client, sobreescreve_recuperar_itens_por_pedido):
      sobreescreve_recuperar_itens_por_pedido([])
      resposta = client.get("/orders/7e290683-d67b-4f96-a940-44bef1f69d21/items")
      assert resposta.status_code == HTTPStatus.OK


   def test_quando_encontrar_pedido_deve_retornar_itens(self, client, sobreescreve_recuperar_itens_por_pedido):
      itens = [
   ...
   def test_quando_fonte_de_pedidos_falha_um_erro_deve_ser_retornado(self, client, sobreescreve_recuperar_itens_por_pedido):
